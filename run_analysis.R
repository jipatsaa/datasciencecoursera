...
#OBTAIN GENERAL INFORMATION ABOUT THE ACTIVITY AND FEATURE CODES
#############################
...
features<-features$V2 #bakarrik behar dugu bigarren zutabea
...
############################
trainfitx<-paste(dirDataSet,"train/X_train.txt",sep="/")
trainProcessedFeatures<-read.table(trainfitx)
testfitx<-paste(dirDataSet,"test/X_test.txt",sep="/")
testProcessedFeatures<-read.table(testfitx)
corpProcessedFeatures<-rbind(trainProcessedFeatures,testProcessedFeatures) #bind Test+Train obtaining the whole corpus (dataSet)
names(corpProcessedFeatures)<-as.character(features) #adding the names of the features to each column values

# We want to process only the (M/m)ean and (S/s)tandard deviation features
# Steps to filter those
# get the indexes of the columns containing (M/m)ean or (S/s)td
...
#OBTAIN CLASS INFORMATION (the class to guess)
#################################
...
#bind the information about the subjects with their corresponding processed movement features and the activity associated to the subject in that moment
